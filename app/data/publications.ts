...
type RasterOpts = { avif?: string; webp?: string; frameZoom?: number };
...
function rasterImage(
  ...
  return {
    ...
    alt,
    ...(typeof opts?.frameZoom === 'number' ? { frameZoom: opts.frameZoom } : {}),
  };
}
...
const architectureNzImage = (filename: string, alt: string, opts?: RasterOpts) =>
  rasterImage(ARCH_NZ_AVIF, ARCH_NZ_WEBP, filename, alt, opts);
...
const groomsGuideImage = (filename: string, alt: string, avifFilename = filename, opts?: RasterOpts) =>
  rasterImage(GROOMS_AVIF, GROOMS_WEBP, filename, alt, { avif: avifFilename, ...opts });
...
const nzwImage = (filename: string, alt: string, opts?: RasterOpts) =>
...
const housesImage = (filename: string, alt: string, opts?: RasterOpts) =>
  rasterImage(HOUSES_AVIF, HOUSES_WEBP, filename, alt, opts);
...
const beautyImage = (filename: string, alt: string, opts?: RasterOpts) =>
  rasterImage(BEAUTY_AVIF, BEAUTY_WEBP, filename, alt, opts);
...
const littleTreasuresImage = (filename: string, alt: string, opts?: RasterOpts) =>
...
const plannerImage = (filename: string, alt: string, opts?: RasterOpts) =>
...
const NZ_WEDDINGS_PLANNER_GALLERY: RasterImageSources[] = [
  ...
  plannerImage(
    ...
    { avif: 'nz-weddings-pre-wedding-celebrations', frameZoom: 1.02 }),
  plannerImage(
...
const pumpkinImage = (filename: string, alt: string, opts?: RasterOpts) =>
...
const PUMPKIN_PATCH_GALLERY: RasterImageSources[] = [
  ...
  pumpkinImage(
    ...
    { avif: 'pumpkin-patch-autumn-cover-back-cover', frameZoom: 1.02 }),
  pumpkinImage(
...
const superLifeImage = (filename: string, alt: string, opts?: RasterOpts) =>
...
const SUPERLIFE_GALLERY: RasterImageSources[] = [
  ...
  superLifeImage(
    ...
    { avif: 'super life koru cover closed copy 2', frameZoom: 1.04 }),
  superLifeImage(
